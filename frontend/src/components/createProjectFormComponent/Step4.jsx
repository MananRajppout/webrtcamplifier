...
const Step4 = ({ formData, setFormData }) => {
  ...
  useEffect(() => {
    const fetchBreakoutRooms = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get-all-breakout-rooms`, {
  ...
  const removeBreakoutRoom = async (index) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/delete-breakout-room/${roomToDelete._id}`);
