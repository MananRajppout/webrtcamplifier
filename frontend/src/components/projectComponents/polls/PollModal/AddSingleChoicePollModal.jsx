...
const AddSingleChoicePollModal = ({ onClose, onSave, project, user }) => {
  const [title, setTitle] = useState(""); // New state for poll title
  const [questions, setQuestions] = useState([
    { question: "", type: "Single Choice", choices: [{ text: "" }] }, // Updated structure
  ]);

  const addQuestion = () => {
    setQuestions([...questions, { question: "", type: "Single Choice", choices: [{ text: "" }] }]);
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <HeadingBlue25px children="Add Single Choice Poll" />
        <InputField
          label="Poll Title"
          type="text"
          value={title}
        ...
        {questions.map((q, qIndex) => (
          <div key={qIndex} className="mt-4">
            ...
            <div className="bg-[#f3f3f3] p-4 mt-2">
              {q.choices.map((choice, cIndex) => ( // Updated to use choices
                <div
                  key={cIndex}
                ...
                >
                  <InputField
                    ...
                    value={choice.text}
                    onChange={(e) =>
                      updateChoice(qIndex, cIndex, e.target.value) // Updated function name
                    }
                  />
                  <FiMinus
                    className="bg-custom-red text-white p-0.5 font-bold rounded-xl cursor-pointer ml-3"
                    onClick={() => removeChoice(qIndex, cIndex)} // Updated function name
                  />
                </div>
              ))}
              <Button
                type="button"
                variant="save"
                children="Add Choice" // Updated button text
                className="py-1 px-5 shadow-[0px_3px_6px_#09828F69] rounded-xl"
                icon={<GoPlus />}
                onClick={() => addChoice(qIndex)} // Updated function name
              />
            </div>
          </div>
        ))}
