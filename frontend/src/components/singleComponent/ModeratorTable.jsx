...
const ModeratorTable = () => {
  ...
  const fetchModerators = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/get-all/moderator?page=1&limit=10`);
  ...
  const handleDeleteModerator = async (moderatorId) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/delete/moderator?id=${moderatorId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (response.ok) {
  ...
  const handleInviteModerator = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/moderator-invitation/link'`, {
        method: 'POST',
        headers: {
