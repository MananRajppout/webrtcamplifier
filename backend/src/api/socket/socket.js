...
const uploadToS3 = async (buffer, mimetype, fileName) => {
  const uniqueFileNamze = `${Date.now()}-${fileName}`;
  const params = {
...
async function checkLiveMeetingExists(identifier, socket, event) {
  ...
  // Check if the identifier is a projectId or meetingId
  if (identifier.projectId) {
    existingLiveMeeting = await LiveMeeting.findOne({
      projectId: identifier.projectId,
    });
  } else {
    existingLiveMeeting = await LiveMeeting.findOne({
      meetingId: identifier.meetingId,
    });
  }
  console.log('existingLiveMeeting',existingLiveMeeting)
...
const setupSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "join-room",
      async (
        { projectId, roomid, name, email, roomname, role, isTechHost },
        callback
      ) => {
        ...
        console.log(
          `User with name ${name} joined meeting ${
            projectId || roomid
          } role ${role}`
        );
    ...
    socket.on("participantWantToJoin", async (data) => {
      const { projectId, meetingId, name, role, email, image } = data;
      ...
      if (image) {
        try {
          const { fileName, fileBase64 } = image; 
      ...
      io.to(projectId || meetingId).emit("participantWaiting", {
        waitingRoom: liveMeeting.waitingRoom,
      });
    });
    ...
    socket.on(      "save-poll-results-csv",
      async (
